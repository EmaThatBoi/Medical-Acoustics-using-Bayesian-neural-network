...
plt.legend()
plt.suptitle("CNN Loss")
plt.title(f"lr={lr}")
plt.grid(True, linestyle='--', linewidth=0.5)
plt.xlim(0,num_epochs)
plt.ylim(0, 3)
#plt.savefig('BCNN_tr_estimate_loss')
plt.show()

plt.plot(train_accuracies, label='Training Accuracy')
plt.plot(valid_accuracies, label='Validation Accuracy')
plt.legend()
plt.suptitle("CNN Accuracy")
plt.title(f"lr={lr}")
plt.grid(True, linestyle='--', linewidth=0.5)
plt.xlim(0,num_epochs)
plt.ylim(0,1)
#plt.savefig('BCNN_tr_accuracy')
plt.show()
...
all_uncertainties = []
barplot_data = []
list_threshold = np.array([0.5, 0.4, 0.3, 0.25])  # certainty = 1 - threshold
epistemica_threshold = np.array([])
aleatoric_threshold = np.arra([])
accuracy = []
...
with torch.no_grad():
    for inputs, labels in test_loader:
        ...
        for threshold in list_threshold:
            high_certainty_indices = np.where(predictive_uncertainty < threshold)[0]
            total_certain_prediction = len(high_certainty_indices)
            all_uncertainties.extend(predictive_uncertainty[high_certainty_indices])
            ...
            plt.ylim(min(predictive_uncertainty), max(predictive_uncertainty))
            plt.show()
...
for i, certainty in enumerate(list((1 - list_threshold) * 100)):
    ax.annotate(num_samples[i], (certainty + 0.3, 100 * balanced_accuracies[i] + 0.2))
